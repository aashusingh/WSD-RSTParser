...
def createnode(node, content, fin2, fin3, fin4):
    ...
    """
    temp_text = fin2.readline().strip().replace('\n','')
    temp_list = temp_text.split(' ')
    node.pos_Text = temp_text
    node.firstPOS = temp_list[0]
    node.lastPOS = temp_list[-1]
    if ( len(temp_list) >= 2):
        node.slastPOS = temp_list[-2]
        node.sfirstPOS = temp_list[1]
        
    temp_text1 = fin3.readline().strip().lower().replace('\n','')
    if temp_text1!= "":
        node.headlist = temp_text1.lower().split(' ')
        if('%%aashu' in node.headlist):
            node.headlist = node.headlist[:-1]
            node.headlist = [x for x in node.headlist if isalpha(x)]
            node.ishead = True
        node.headtuple = tuple(set(node.headlist))
    else:
        node.headlist = []
        node.headtuple = tuple()
    
    temp_text2 = fin4.readline().strip().lower().replace('\n','')
    if temp_text2 != "":
        node.senseSingle_list = [temp_text2.lower()]
        node.senseSingle_tuple = tuple(set(node.senseSingle_list))
    else:
        node.senseSingle_list = []
        node.senseSingle_tuple = tuple()
    for c in content:
...
def buildtree(text, fname):
    ...
    """
    s = fname.split('.')[:-1]
    fpos_name = '.'.join((str(e) for e in s))
    with open(fpos_name+ ".mypos") as fin2:
        with open(fpos_name+ ".mylasttry_dep") as fin3:
            with open(fpos_name + ".myindpart3") as fin4:
        ...
        #                 pos_text = fin2.readline()
                        label = content.pop(0)
                        if label == 'Root':
                            node = SpanNode(prop=label)
                            node = createnode(node, content, fin2, fin3, fin4)
                            stack.append(node)
                        elif label == 'Nucleus':
                            node = SpanNode(prop=label)
                            node = createnode(node, content, fin2, fin3, fin4)
