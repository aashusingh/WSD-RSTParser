    ...
    def report(self):
        ...
        for level in self.levels:
            if 'span' == level:
                ...
                f1 = (2 * p * r) / (p + r)
                print 'F1 score of RSTparser on span level is {0:0.3f}'.format(f1)
            elif 'nuclearity' == level:
                p = numpy.array(self.nuc_perf.percision).mean()
                r = numpy.array(self.nuc_perf.recall).mean()
                f1 = (2 * p * r) / (p + r)
                print 'F1 score of RSTparser on nuclearity level is {0:0.3f}'.format(f1)
            elif 'relation' == level:
                p = numpy.array(self.rela_perf.percision).mean()
                r = numpy.array(self.rela_perf.recall).mean()
                f1 = (2 * p * r) / (p + r)
                print 'F1 score of RSTparser on relation level is {0:0.3f}'.format(f1)
